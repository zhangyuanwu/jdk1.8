...
/**
 * 用于{@link Method}和{@link Constructor}的常用功能的共享超类。
 *
 * @since 1.8
 */
public abstract class Executable extends AccessibleObject implements Member, GenericDeclaration {
	...
	/**
	 * 返回表示声明此对象表示的可执行文件的类或接口的{@code Class}对象。
	 */
	@Override
	...
	private synchronized Map<Class<? extends Annotation>, Annotation> declaredAnnotations() {
		if (declaredAnnotations == null) {
			...
			if (root != null) {
				...
				declaredAnnotations = AnnotationParser.parseAnnotations(getAnnotationBytes(), sun.misc.SharedSecrets.getJavaLangAccess().getConstantPool(getDeclaringClass()), getDeclaringClass());
			}
		}
