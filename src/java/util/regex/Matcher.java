...
public final class Matcher implements MatchResult {
	...
	public MatchResult toMatchResult() {
		Matcher result = new Matcher(parentPattern, text.toString());
		result.first = first;
		result.last = last;
	 ...
	 * <a href="Pattern.html#cg">Capturing groups</a> are indexed from left to
	 * right, starting at one. Group zero denotes the entire pattern, so the
	 * expression <i>m.</i><tt>end(0)</tt> is equivalent to
	 * <i>m.</i><tt>end()</tt>.
	 * </p>
	 *
	 * @param group
	 ...
	 * during the previous match: Each occurrence of
	 * <tt>${</tt><i>name</i><tt>}</tt> or <tt>$</tt><i>g</i> will be replaced
	 * by the result of evaluating the corresponding {@link #group(String)
	 * group(name)} or {@link #group(int) group(g)} respectively. For
	 * <tt>$</tt><i>g</i>, the first number after the <tt>$</tt> is always
