...
class ArrayPrefixHelpers {
			...
			outer: while (((l = t.lo) >= 0) && ((h = t.hi) <= a.length)) {
				if ((h - l) > th) {
					...
					if (lt == null) { // first pass
						int mid = (l + h) >>> 1;
						f = rt = t.right = new CumulateTask<>(t, fn, a, org, fnc, th, mid, h);
						t = lt = t.left = new CumulateTask<>(t, fn, a, org, fnc, th, l, mid);
	...
	static final class LongCumulateTask extends CountedCompleter<Void> {
		...
		public LongCumulateTask(LongCumulateTask parent, LongBinaryOperator function, long[] array, int lo, int hi) {
			...
			this.hi = fence = hi;
			int p;
			threshold = (p = (hi - lo) / (ForkJoinPool.getCommonPoolParallelism() << 3)) <= MIN_PARTITION ? MIN_PARTITION : p;
		}
		...
		public final void compute() {
			...
			final long[] a;
			if (((fn = function) == null) || ((a = array) == null)) {
				throw new NullPointerException(); // hoist checks
			}
			int th = threshold, org = origin, fnc = fence, l, h;
