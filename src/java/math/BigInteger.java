...
public class BigInteger extends Number implements Comparable<BigInteger> {
	 ...
	 *            1/2<sup>{@code certainty}</sup>). The execution time of this
	...
	private BigInteger divideKnuth(BigInteger val) {
		MutableBigInteger q = new MutableBigInteger(), a = new MutableBigInteger(mag), b = new MutableBigInteger(val.mag);
	...
	public BigInteger modPow(BigInteger exponent, BigInteger m) {
		...
		BigInteger base = ((signum < 0) || (compareTo(m) >= 0) ? mod(m) : this);
		BigInteger result;
		if (m.testBit(0)) { // odd modulus
			...
			BigInteger base2 = ((signum < 0) || (compareTo(m1) >= 0) ? mod(m1) : this);
	 ...
	 *            a measure of the uncertainty that the caller is willing to
	 *            tolerate: if the call returns {@code true} the probability
	 *            that this BigInteger is prime exceeds (1 -
	 *            1/2<sup>{@code certainty}</sup>). The execution time of this
